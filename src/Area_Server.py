...
class Server:
    ...
    def handle_router_connection(self):
        ...
        while True:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.connect(("localhost", self.canton_port))
                time.sleep(int(sys.argv[2]))
                s.sendall(b"20800020000200002000SoyElMensaje")
            self.router_socket.sendall(b"" + self.messageQueue.get())
            break
        """
    ...
    def process_and_enqueue(self, string_package):
        ...
        for i in range(len(list_of_lines)):
            ...
            self.router_socket.sendall(router_package.encode())
...
def main():
    ...
    # my_server.create_router_thread()
