...
class Intra_provincial_router:
    def __init__(self, node, subnode):
        ...
        self.port = None
        self.im_border_router = (
            False  # Bool variable if i'm the router connected with the province.
        )
                ...
                if line.find(logical_IP) != -1:
                    if line[0:7] == logical_IP:
                        ...
                        connections["Port"].append(
                            line[first_port_byte : first_port_byte + 5]
                        )
        ...
        try:
            ...
            for index in range(len(ports)):
                port_thread = threading.Thread(
                    target=self.router_init, args=(do_Bind[index], ports[index], index)
                )
    ...
    def updatePackage(self, package):
        jumps = int(package[3:5])
        jumps += 1
        if jumps < 10:
            jumps = "0" + str(jumps)
        jumps = str(jumps)
        package = package[0:3] + jumps + package[5:]
        package = (
            package[0:10]
            + (self.IP_Address[0:2] + self.IP_Address[3:5] + self.IP_Address[6])
            + package[15:]
        )
        return package
    ...
    def redirect_packages(self):
        ...
        while True:
            ...
            package = self.updatePackage(package)
            if not package == "shutdown":
                # send to distance-vector algorithm
                d_package = self.decompose_message(package)
    ...
    def create_routing_table(self):
        ...
        neighbors, path = my_router.dijkstra(
            my_router.my_graph, my_router.IP_Address.replace(".", " ")
        )
        ...
        for items in neighbors.keys():
            ...
            self.memory.setRoutingNeighborFor(
                address, str(path[items][0]).replace(" ", "")
            )
            ...
            self.memory.setRoutingPortFor(
                address, str(self.routing_tables_ports[current_counter])
            )
    ...
    def analize_message(self, msg_dictionary):
        ...
        if my_IP_info[0] == msg_IP_info[0]:
            ...
            if my_IP_info[1] == msg_IP_info[1]:
                ...
                print(
                    "Province and Canton are the same, send to specified health area."
                )
            else:
                ...
                print(
                    "I don't have a connection with that router, must check routing table."
                )
        # Different province.
        else:
