...
class Inter_provincial_router:
  ...
  def handle_connection(self, client_socket, client_address, thread_num):
    ...
    first_route_package = self.distance_vector_algorithm()
    client_socket.sendall(first_route_package.encode("utf-8"))
    while True:
      try:
    ...
    while True:
      ...
      if (not package == "shutdown"):
        ...
        if (d_package["ID"] == "0"):
          ...
          if (not routing_package == None):
            print("Table updated: send routing package to all neighbors")
            for index in range(len(self.output_queues)):
              self.output_queues[index].put(routing_package)
          ...
          if (not self.its_me_destiny(d_package["DestinyIP"])):
            ...
            redirect_IP = self.memory.getRoutingNeighborFor(
              self.translate_string_IP_to_int(self.its_package_from_this_province(d_package["DestinyIP"]), "."))
  ...
  def its_package_from_this_province(self, package_destiny_ip):
    logical_destiny_ip = package_destiny_ip
    if(not package_destiny_ip[:2] == self.logic_IP[:2]):
      logical_destiny_ip = package_destiny_ip[0:2] + "00" + "0"
    
    return  logical_destiny_ip
  ...
  def initialize_routing_table(self):
    ...
    # Send table to neighbors (Exception with cantonal router)
      

  # Updates table
  def distance_vector_algorithm(self,routing_package):
    new_routing_package = None
    table_updated = False
    my_IP = int(self.translate_string_IP_to_int(self.logic_IP,"."))
    
    # Compare every table entry with my entry
    rp_index = 20
    other_routing_table = self.get_other_table_info(routing_package)
    # other_routing_table = {"Destiny":[],"Cost":[],"Route":[]}  Destiny = IP
    for index in range(len(other_routing_table["Destiny"])):
      # Values of my table
      if (other_routing_table["Destiny"][index][2:4] == "01"):
        my_table_route = self.memory.getRoutingNeighborFor(self.translate_string_IP_to_int(other_routing_table["Destiny"][index]))
        my_table_cost = self.memory.getRoutingWeightFor(self.translate_string_IP_to_int(other_routing_table["Destiny"][index]))
      elif (other_routing_table["Destiny"][index][2:4] == "00"):
        my_table_route = self.memory.getRoutingNeighborFor(self.translate_string_IP_to_int(other_routing_table["Destiny"][index]))
        my_table_cost = self.memory.getRoutingWeightFor(self.translate_string_IP_to_int(other_routing_table["Destiny"][index]))

      # Compare entry values 05
      if (not other_routing_table["Cost"][index] == "In" and other_routing_table["Cost"][index] == "00"):
        if (int(other_routing_table["Cost"][index]) + int(other_routing_table["Cost"][0]) < int(my_table_cost)):
          table_updated = True
          new_cost = int(other_routing_table["Cost"][index]) + int(other_routing_table["Cost"][0])
          if (new_cost <= 9):
            new_cost_str = "0"+str(new_cost)
          else:
            new_cost_str = str(new_cost)
          self.memory.setRoutingWeightFor(self.translate_string_IP_to_int(other_routing_table["Destiny"][index]), new_cost_str)
          self.memory-setRoutingNeighborFor(
            self.translate_string_IP_to_int(other_routing_table["Destiny"][index]), self.translate_string_IP_to_int(other_routing_table["Route"][index]))

    # if table was updated, then build package to be sent to neighbors
    if (table_updated):
      new_routing_package = build_distance_vector_package()
      # Send package to all connections (Exception with cantonal router)
  ...
  def build_distance_vector_package(self):
    ...
    header = {"ID":"0", "MaxJumps":"08","CurrentJumps":"00", 
      "OriginIP":my_IP,
      "LastIP":my_IP,
      "DestinyIP":"00000"}
    # Build content
    ...
    #self.get_other_table_info(routing_package)
    return routing_package

  def get_other_table_info(self, routing_package):
    other_routing_table = {"Destiny":[],"Cost":[],"Route":[]}
    rp_index = 20
    other_routing_table["Destiny"].append(routing_package[5:10])
    other_routing_table["Cost"].append("00")
    other_routing_table["Route"].append("_____")
    for index in range(16):
      # Values of other table (package table)
      other_IP = routing_package[rp_index:rp_index + 5]
      rp_index += 5
      other_route = routing_package[rp_index:rp_index + 5]
      rp_index += 5
      other_cost = routing_package[rp_index:rp_index+2]
      rp_index += 2
      other_routing_table["Destiny"].append(other_IP)
      other_routing_table["Cost"].append(other_cost)
      other_routing_table["Route"].append(other_route)
    #for key,value in other_routing_table.items():
    #  print(key, ' : ',value)
    return other_routing_table
