...
class MMU:
    def __init__(self, myLogicalIp):
        ...
        self.__ROUTING_INFO_DISK_SLOTS = 40 + self.__IS_PROVINCIAL * (
            -24
        )  # 40 if cantonal, 16 if provincial
        self.__ROUTING_INFO_RAM_SLOTS = 16 + self.__IS_PROVINCIAL * (
            -8
        )  # 16 if cantonal, 8 if provincial
        ...
        self.tlb = TLB.TLB()
        self.ramClock = ClockPagination.ClockPagination(
            self.__ROUTING_INFO_RAM_SLOTS
        )  # RAM_SIZE
    ...
    def translateDiskAddrToProvincialIp(
        self, diskAddr
    ):  # TODO CASE WHERE DISKADDR == 0
        ip = None
        if diskAddr == 0:
    ...
    def translateCantonalIpToDiskAddr(self, ip):  # 3174
        myAddr = self.__MY_LOGICAL_IP
        base = math.floor(myAddr / 1000) * 1000  # 1000 router 1 1 0
        split = (myAddr - base) / 10  # 1 router 1 1 0
        units = ip % 10  # 4 r 1 1 0
        diff = ip - myAddr  # 2164 r 1 1 0
        tens = math.floor(diff / 10)  # 216 r 1 1 0
    ...
    def translateDiskAddrToCantonalIp(self, diskAddr):
        ...
        units = (diskAddr >= split and diskAddr <= split + 7) * (diskAddr - split + 1)
        ip = (
    ...
    def getRoutingNeighborFor(self, ip):
        ...
        return (
            self.safeRamGetIp(
                self.askTlbForPageAddress(math.floor(diskAddr / 2)), diskAddr % 2
            )[5:10]
        ).decode("utf-8")
    ...
    def safeRamSetIp(self, data, virtualAddr, halfPage, startingByte):
        if type(data) != bytearray:
            data = data.encode("utf-8")
        ramAddr = (
            self.translateAddress(virtualAddr)
            + halfPage * self.__IPADDR_SIZE
            + startingByte
        )
...
print(mmu.getRoutingNeighborFor(3174))
